...
import { FaClipboard, FaSun , FaMoon , FaEye , FaWindowClose } from "react-icons/fa";
...
const Sidebar = ({ isSideBarOpen, setIsSideBarOpen }) => {
    ...
    return (
          ...
          <div>
                ...
                <div className="  dropdown-borad flex flex-col h-[70vh]  justify-between ">
                  <div>
                    {boards.map((board, index) => (
                      ...
                      >
                        <FaClipboard className="h-4"></FaClipboard>{" "}
            ...
            {isSideBarOpen ? (
              <div
                ...
                className=" flex  items-center mt-2  absolute bottom-12  text-lg font-bold  rounded-r-full hover:text-[#635FC7] cursor-pointer mr-6 mb-8 px-8 py-4 hover:bg-[#635fc71a] dark:hover:bg-white  space-x-2 justify-center  my-4 text-gray-500 "
              >
                <FaWindowClose></FaWindowClose>
                {isSideBarOpen && <p> Hide Sidebar </p>}
              </div>
            ) : (
                <div className=" absolute p-5 flex  items-center " onClick={() => toggleSidebar()}>
                  <FaEye className="h-4 text-white "></FaEye>
              </div>
            )}
          </div>
        </div>
        ...
        {isBoardModalOpen && (
          <AddBoard
            type="add"
            setModalOpen={setIsBoardModalOpen}
          />
        )}
      </div>
    );
